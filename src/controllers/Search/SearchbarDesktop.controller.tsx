...
import { SearchbarDesktopComponent } from './../../components/Search'
...
export const SearchbarDesktop = (props: any) => {
  ...
  const searchProperties = () => {
    if (/* query.get("country") */ query.get("location_id") && query.get("property_types")) {
      ...
      const currency = query.get("currency") || "MXN";
      const priceFrom = parseInt(query.get("price_from") || "");
      const bathroomAmount = parseInt(query.get('bathroom_amount') || "0" )
      const suiteAmount = parseInt(query.get('suite_amount') || "0" )
      const tagsList = JSON.parse(query.get("tags") || "[5]");
      const locationId = JSON.parse(query.get("location_id") || "[]");
      history.push(`/search?location_id=${locationId}&property_types=[${propertyTypeList}]&tags=[${tagsList}]&suite_amout=${suiteAmount}&bathroom_amount=${bathroomAmount}&price_from=${priceFrom}}`)
    }
  }
