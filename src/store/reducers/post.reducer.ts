...
export default function Reducer (state: State = {
  ...
  switch (action.type) {
    ...
    case 'GET_' + ENTITY.ENTITY_MAYUS + '_BY_CATEGORY_PENDING': {
    ...
    case 'GET_' + ENTITY.ENTITY_MAYUS + '_BY_CATEGORY_REJECTED': {
    ...
    case 'GET_' + ENTITY.ENTITY_MAYUS + '_BY_CATEGORY_FULFILLED': {
      ...
      return {
        ...
        status: action.payload.data.status,
        message: action.payload.data.message,
        data: Symbol.iterator in Object(state.data) ? [...state.data, ...action.payload.data.result] : action.payload.data.result
      };
