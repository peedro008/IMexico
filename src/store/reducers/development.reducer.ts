...
const ENTITY = entities.DEVELOPMENT;
...
export default function Reducer(
	...
	switch (action.type) {
		case "GET_" + ENTITY.ENTITY_MAYUS + "_WITH_AUTH_PENDING": {
			return {
				...state,
				fetching: true,
			};
		...
		case "GET_" + ENTITY.ENTITY_MAYUS + "_WITH_AUTH_REJECTED": {
		...
		case "GET_" + ENTITY.ENTITY_MAYUS + "_WITH_AUTH_FULFILLED": {
			return {
				...state,
				fetching: false,
				fetched: true,
		...
		case "GET_" + ENTITY.ENTITY_MAYUS_PLURAL + "_PENDING": {
			return {
				...state,
				fetching: true,
			};
		...
		case "GET_" + ENTITY.ENTITY_MAYUS_PLURAL + "_REJECTED": {
			let data: any = action?.payload?.response?.data || {};
			let status: any = action?.payload?.response?.status || 500;
			let message: string = action?.payload?.response?.data?.message || "";
		...
		case "GET_" + ENTITY.ENTITY_MAYUS_PLURAL + "_FULFILLED": {
			return {
				...state,
				fetching: false,
				fetched: true,
