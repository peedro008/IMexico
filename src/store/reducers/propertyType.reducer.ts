...
const ENTITY = entities.PROPERTYTYPE
...
export default function Reducer(state: State = {
  ...
  switch (action.type) {
    ...
    case 'GET_' + ENTITY.ENTITY_MAYUS + '_FULFILLED': {
      ...
      /* const quickSort: any = ( [ x = [], ...xs ]: any ) => {
        return ( x.length === 0 ) ? [] : [
          ...quickSort( xs.filter( (y: any) => y.type <= x.type ) ),
          x,
          ...quickSort( xs.filter( (y: any) => y.type > x.type ) )
        ];
      }

      const result = quickSort(action.payload.data.result[0].items); */

      /* action.payload.data.result[0].items = result */
    ...
    case 'GET_' + ENTITY.ENTITY_MAYUS + '_BY_COUNTRY_PENDING': {
    ...
    case 'GET_' + ENTITY.ENTITY_MAYUS + '_BY_COUNTRY_REJECTED': {
    ...
    case 'GET_' + ENTITY.ENTITY_MAYUS + '_BY_COUNTRY_FULFILLED': {
