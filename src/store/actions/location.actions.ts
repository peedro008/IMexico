...
export function getById(id: number) {
  ...
  let payload: any = axios.get(
    ...
    "/" +
    ENTITY.ENTITY +
    `/${id}?project=${JSON.stringify(project)}&match=${JSON.stringify(
      match
    )}&sort=${JSON.stringify(sort)}&group=${JSON.stringify(
      group
    )}&limit=${limit}&skip=${skip}`,
...
export function getByCountry(countryId: number) {

  let project = {};
  let match = {
    data: {
      current_localization_id: 14,
  ...
  let payload: any = axios.get(
    ...
    "/" +
    ENTITY.ENTITY +
    `/withauth/${config.company}?project=${JSON.stringify(project)}&match=${JSON.stringify(
      match
    )}&sort=${JSON.stringify(sort)}&group=${JSON.stringify(
      group
    )}&limit=${limit}&skip=${skip}`,
