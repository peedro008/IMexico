...
const ENTITY = entities.PROPERTYTAG;

export function getAll(location: number, propertyType: number) {

  let project = {};
  let match = {
    data: {
      current_localization_id: [location],
      current_localization_type: "division",
      price_from: 0, price_to: 4500000,
  ...
  let payload: any = axios.get(
    config.url +
    `/${ENTITY.ENTITY}/withauth/imexico` +
